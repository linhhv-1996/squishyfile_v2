...
// changing.
let NATIVE_SCALE = 2; // Will be set dynamically
...
async function loadModel(scaleFactor: number) {
  if (!session || currentSessionScale !== scaleFactor) {
    if (session) {
      session = null;
    }
    currentSessionScale = scaleFactor;
    const modelPath = `/ai/espcn_x${scaleFactor}.onnx`;
    ...
    try {
      self.postMessage({ type: 'LOG', data: `Loading ONNX model ${modelPath} (WebGPU, graph capture)...` });
      ...
      try {
        self.postMessage({ type: 'LOG', data: `Graph capture unavailable, loading ${modelPath} WebGPU without it...` });
        session = await InferenceSession.create(modelPath, {
          executionProviders: ['webgpu'],
          preferredOutputLocation: 'gpu-buffer'
        });
        ...
        self.postMessage({ type: 'LOG', data: `WebGPU failed, falling back to WASM for ${modelPath}.` });
        session = await InferenceSession.create(modelPath, {
...
async function processFrame(sample: VideoSample, scaleFactor: number): Promise<OffscreenCanvas> {
  ...
  const sess = await loadModel(scaleFactor);
